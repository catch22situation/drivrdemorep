...
import com.catch22.drivrdemo.config.KafkaProperties;
...
class DrivrdemoKafkaResourceIT {
    ...
    void setup() {
        ...
        DrivrdemoKafkaResource kafkaResource = new DrivrdemoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/drivrdemo-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/drivrdemo-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
